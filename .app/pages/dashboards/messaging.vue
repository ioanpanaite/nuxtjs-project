    ...
    <div class="bg-muted-100 dark:bg-muted-900 flex min-h-screen">
      ...
      <div
        ...
        <div class="flex h-full flex-col justify-between">
          <div class="flex flex-col">
            <div class="ltablet:w-full flex h-16 w-16 shrink-0 items-center justify-center lg:w-full">
            ...
            <div class="ltablet:w-full flex h-16 w-16 shrink-0 items-center justify-center lg:w-full">
              <a href="#"
                ...
                title="Back" @click.prevent="$router.back()">
          ...
          <div class="flex flex-col">
            <div class="flex h-16 w-full items-center justify-center">
              <button type="button"
                class="text-muted-400 hover:text-primary-500 hover:bg-primary-500/20 flex h-12 w-12 items-center justify-center rounded-2xl transition-colors duration-300"
                title="Search" @click="open('search')">
      ...
      <div
        ...
        <div class="flex h-full flex-col">
          <button class="flex h-16 w-16 shrink-0 items-center justify-center sm:w-20">
            <span class="bg-primary-500 flex h-10 w-10 items-center justify-center rounded-full text-white">
              <Icon name="lucide:plus" class="h-4 w-4" />
            </span>
          </button>
          <!-- List -->
          <a v-for="conversation in conversations" :key="conversation.id" href="#"
            class="flex h-16 w-16 shrink-0 items-center justify-center border-s-2 sm:w-20" :class="activeConversation === conversation.id
                ? 'border-primary-500'
                : 'border-transparent'
      ...
      <div class="relative w-full transition-all duration-300" :class="expanded
        ...
        <div class="flex w-full flex-col">
          ...
          <!-- Body -->
          <div ref="chatEl" class="relative h-[calc(100vh_-_128px)] w-full p-4 sm:p-8"
            ...
            <div
              ...
              <div class="mt-12 space-y-12">
                ...
                <div class="ms-auto flex w-full max-w-md flex-row-reverse justify-end gap-4">
                  <BasePlaceload class="h-8 w-8 shrink-0 rounded-full" :width="32" :height="32" />
                  <div class="grow space-y-2">
                    <BasePlaceload class="ms-auto h-3 w-full max-w-[16rem] rounded" />
                    <BasePlaceload class="ms-auto h-3 w-full max-w-[12rem] rounded" />
                  </div>
                </div>
                <div class="ms-auto flex w-full max-w-md flex-row-reverse justify-end gap-4">
                  <BasePlaceload class="h-8 w-8 shrink-0 rounded-full" :width="32" :height="32" />
                  <div class="grow space-y-2">
                    <BasePlaceload class="ms-auto h-3 w-full max-w-[14rem] rounded" />
                    <BasePlaceload class="ms-auto h-3 w-full max-w-[8rem] rounded" />
                  </div>
                </div>
                <div class="flex w-full max-w-md gap-4">
                  <BasePlaceload class="h-8 w-8 shrink-0 rounded-full" :width="32" :height="32" />
            ...
            <div v-if="!loading" class="space-y-12">
              <div v-for="(item, index) in selectedConversation?.messages" :key="index" class="relative flex w-full gap-4"
                :class="[
                ...
                <template v-if="item.type !== 'separator'">
                  ...
                  <div class="flex max-w-md flex-col">
                    <div class="bg-muted-200 dark:bg-muted-800 rounded-xl p-4" :class="[
                      item.type === 'received' ? 'rounded-ss-none' : '',
                      item.type === 'sent' ? 'rounded-se-none' : '',
                    ]">
                      <p class="font-sans text-sm">{{ item.text }}</p>
                    </div>
                    <div class="text-muted-400 mt-1 font-sans text-xs"
                    ...
                    <div v-if="item.attachments.length > 0" class="mt-2 space-y-2">
                      <template v-for="(attachment, idx) in item.attachments" :key="idx">
                        ...
                        <NuxtLink :to="attachment.url" v-else-if="attachment.type === 'link'"
                ...
                <div v-else>
                  <div class="absolute inset-0 flex items-center" aria-hidden="true">
                    <div class="border-muted-300/50 dark:border-muted-800 w-full border-t"></div>
                  </div>
                  <div class="relative flex justify-center">
                    <span class="bg-muted-100 dark:bg-muted-900 text-muted-400 px-3 font-sans text-xs uppercase">
                      {{ item.time }}
                    </span>
                  </div>
                </div>
          ...
          <!-- Compose -->
          <form method="POST" action="" @submit.prevent="submitMessage"
            class="bg-muted-100 dark:bg-muted-900 flex h-16 w-full items-center px-4 sm:px-8">
            <div class="relative w-full">
              <BaseInput v-model.trim="message" :disabled="messageLoading" shape="full" :classes="{
        ...
        <div class="relative flex w-full flex-col px-8">
          ...
          <div v-else class="mt-8">
            ...
            <div class="text-center">
              ...
              <div class="divide-muted-200 dark:divide-muted-700 flex items-center justify-center divide-x">
                <div class="flex items-center justify-center gap-2 px-4">
                  <Icon name="ph:timer-duotone" class="text-muted-400 h-4 w-4" />
                  <span class="text-muted-400 font-sans text-xs">
                    Age: {{ selectedConversation?.user.age }}
                  </span>
                </div>
                <div class="flex items-center justify-center gap-2 px-4">
                  <Icon name="ph:map-pin-duotone" class="text-muted-400 h-4 w-4" />
                  <span class="text-muted-400 font-sans text-xs">
              ...
              <div class="mt-6">
                ...
                <button type="button" class="text-primary-500 mt-3 font-sans text-sm underline-offset-4 hover:underline">
