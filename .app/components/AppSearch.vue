...
<template>
  <div>
    <TairoModal :classes="{
    ...
    }" :open="isOpen" size="md" @close="isOpen = false">
      <BaseFocusLoop next-keys="ArrowDown" prev-keys="ArrowUp">
        <div class="px-2 pb-2">
          <BaseInput type="search" shape="curved" icon="lucide:search" v-model="search"
            placeholder="Ex: button or analytics..." v-focus color-focus>
            <template #label>
        ...
        <div v-if="contentDocsResults?.length">
          <div class="px-3 pt-2">
            <span class="nui-text-500 text-[0.65rem] font-medium uppercase tracking-wider">
              Documentation Hub
            </span>
          </div>
          <ul>
            <li v-for="page in contentDocsResults" :key="page?._path" class="">
              <AppSearchResult :to="page?._path" :search="search" :title="page?.title" :subtitle="page?._path"
                :icon="page?.icon?.src" @click.passive="onClick" />
            </li>
          </ul>
        </div>
        ...
        <div v-if="demoPagesResults?.length">
          ...
          <ul>
            <li v-for="page in demoPagesResults" :key="page?.name" class="">
              ...
              }" :search="search" :title="page?.meta?.preview?.title" :subtitle="page?.meta?.preview?.description"
      ...
      <div class="flex flex-col items-center py-3 text-center">
        <div class="scale-[0.8]">
          <BaseText size="xs" weight="medium" class="text-muted-400">Search by</BaseText>
